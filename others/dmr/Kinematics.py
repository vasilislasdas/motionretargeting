...
class ForwardKinematics:
    ...
    def forward(self, rotation: torch.Tensor, position: torch.Tensor, offset: torch.Tensor, order='xyz', quater=False, world=True):
        ...
        # print(f"rotation:{rotation.shape}, position:{position.shape}, offset:{offset.shape}")
        ...
        for i, pi in enumerate(self.topology):
            ...
            # TODO: i have modified this code
            tmp1 = transform[..., pi, :, :].clone()
            tmp2 = transform[..., i, :, :].clone()
            transform[..., i, :, :] = torch.matmul(tmp1, tmp2)
