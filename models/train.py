...
def train_model( model, data_loader, rec_loss_module, time_loss_module, device = 'cpu', num_epochs=1):
    ...
    # create optimizer
    # add learnable weights for the losses
    #weight_motions = nn.Parameter(torch.tensor(100).float())
    # optimizer = torch.optim.Adam(list(model.parameters()) + [weight], lr=0.001)
    ...
    for epoch in (range(num_epochs)):
        ...
        for batch in train_iterator:
            ...
            # predict
            prediction,_,_,_,_,_ = model( input_seq, target_seq, input_skeleton, output_skeleton )
            # print( f"Predictions:{prediction.shape}")
            # print( f"expected_seq:{expected_seq.view( expected_seq.shape[0], -1 ).shape}")
            ...
            # self-loss
            self_loss = rec_loss_module( prediction, expected_seq.view( expected_seq.shape[0], -1 ) )
            ...
            # add losses
            # total_loss = self_loss_positions + self_loss_rotations + time_loss
...
def evaluate_mixamo():
    ...
    # fetch first sample to get dimensions of the motion and the skeleton+min_max
    motion_first_sample = dataset[0][0]
    seq_len, input_dim = motion_first_sample.shape
    output_dim = input_dim
    minumum = motion_first_sample[-1]
    maximum = motion_first_sample[-2]
    ...
    for i in range( seq_len - 1):
        ...
        # predict
        prediction, _, _, _, _, _ = retargeter( input_seq, target_seq, input_skeleton, output_skeleton)
